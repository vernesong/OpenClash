...
. /usr/share/openclash/openclash_ps.sh
   ...
   set_lock() {
      exec 879>"/tmp/lock/openclash_chn6.lock" 2>/dev/null
      flock -x 879 2>/dev/null
   }

   del_lock() {
      flock -u 879 2>/dev/null
      rm -rf "/tmp/lock/openclash_chn6.lock"
   }

   START_LOG="/tmp/openclash_start.log"
   LOGTIME=$(date "+%Y-%m-%d %H:%M:%S")
   LOG_FILE="/tmp/openclash.log"
   china_ip_route=$(uci get openclash.config.china_ip6_route 2>/dev/null)
   ...
   if [ "$?" -eq "0" ] && [ -s "/tmp/china_ip6_route.txt" ]; then
      ...
      #预处理
      echo "create china_ip6_route hash:net family inet6 hashsize 1024 maxelem 65536" >/tmp/china_ip6_route.list
   ...
   rm -rf /tmp/china_ip6_route* >/dev/null 2>&1
   echo "" >$START_LOG
   del_lock