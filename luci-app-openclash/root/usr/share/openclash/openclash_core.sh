         ...
         "TUN")
            ...
            if [ "$github_address_mod" != "0" ]; then
               if [ "$github_address_mod" == "https://cdn.jsdelivr.net/" ]; then
			      ...
			      if [ "$?" != "0" ]; then
			         curl -sL -m 30 --speed-time 15 --speed-limit 1 --retry 2 https://osdn.net/projects/openclash/storage/"$RELEASE_BRANCH"/core-lateset/premium/clash-"$CPU_MODEL"-"$CORE_LV".gz -o /tmp/clash_tun.gz >/dev/null 2>&1
			      fi
			   ;;
         "Meta")
            ...
            if [ "$?" != "0" ]; then
               curl -sL -m 30 --speed-time 15 --speed-limit 1 --retry 2 https://osdn.net/projects/openclash/storage/"$RELEASE_BRANCH"/core-lateset/meta/clash-"$CPU_MODEL".tar.gz -o /tmp/clash_meta.tar.gz >/dev/null 2>&1
            fi
			   ;;
			   *)
			      ...
			      if [ "$?" != "0" ]; then
			         curl -sL -m 30 --speed-time 15 --speed-limit 1 --retry 2 https://osdn.net/projects/openclash/storage/"$RELEASE_BRANCH"/core-lateset/dev/clash-"$CPU_MODEL".tar.gz -o /tmp/clash.tar.gz >/dev/null 2>&1
			      fi
			esac
