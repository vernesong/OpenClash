...
. /usr/share/openclash/log.sh
   ...
   if [ -z "$GEOSITE_CUSTOM_URL" ]; then
      ...
      if [ "$?" -ne "0" ]; then
         curl -sL --connect-timeout 5 -m 30 --speed-time 15 --speed-limit 1 --retry 2 "https://mirror.iscas.ac.cn/osdn/storage/g/v/v2/v2raya/dists/v2ray-rules-dat/geosite.dat" -o /tmp/GeoSite.dat >/dev/null 2>&1
      fi
   else
