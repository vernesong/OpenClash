...
# The proxies and proxy-providers blocks are written by uci.rb here: the package is read
# once and the fragments are written in a single pass.
# The GC limits keep the heap small, the default growth factor peaks higher and needs less
# time.
...
ruby -ryaml -rYAML -I "/usr/share/openclash" -E UTF-8 -e "
   require 'uci';
   begin
      UCI.write_proxies('$CONFIG_FILE', '${CONFIG_NAME}');
   rescue ::Exception => e
      YAML.LOG_ERROR('Resolve Proxies Failed,【${CONFIG_NAME} - ' + e.message + '】');
   end;
" 2>/dev/null >> $LOG_FILE
