...
if [ "$create_config" != "0" ] && [ "$servers_if_update" != "1" ] && [ -z "$if_game_proxy" ]; then
   ...
   if [ "$config_hash" != "false" ] && [ -n "$config_hash" ]; then
      config_hash=$(ruby_cover "$CONFIG_FILE" "['proxies']" "$SERVER_FILE" "['proxies']")
      config_hash=$(ruby_cover "$CONFIG_FILE" "['proxy-providers']" "$PROXY_PROVIDER_FILE" "['proxy-providers']")
      config_hash=$(ruby_cover "$CONFIG_FILE" "['proxy-groups']" "/tmp/yaml_groups.yaml" "['proxy-groups']")
   else
