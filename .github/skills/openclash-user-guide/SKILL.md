...
instructions: |
  ...
  2. When users report any issue (cannot access internet, DNS failure, startup failure, etc.), FIRST ask them to generate a debug log — either via LuCI (运行日志 → 调试日志 → 生成日志) or SSH (`/usr/share/openclash/openclash_debug.sh`). The debug log contains dependency checks, config, firewall rules, and system info in one step.
...
**排查优先级（从快到慢，逐层递进）**：
① **先要日志** — 用户报告问题时，首先让用户生成调试日志（LuCI「运行日志 → 调试日志 → 生成日志」，或 SSH `/usr/share/openclash/openclash_debug.sh` → `/tmp/openclash_debug.log`；含依赖检查、配置、防火墙规则、系统信息等 30 个章节）
② **日志不足时给命令** — 对照 `14-diagnostics.md` §14.2 决策树，给精确 CLI 命令让用户执行（🟢 安全查询优先，🟡/🔴 附风险说明）
...
| **配置给路径** | 功能开关/参数调整（如何开启/关闭/设置选项）→ 直接给 LuCI Web 界面操作路径（如「服务 → OpenClash → 插件设置 → 流量控制」）；仅在用户明确要求 CLI 操作或 LuCI 不可用时才提供终端命令。 |
...
| **查 Issues，不闭门造车** | 本文档未覆盖或 `03-errors.md` 无匹配报错时，**必须搜索 Issues**：插件侧（配置/订阅/防火墙/UI）搜 [OpenClash Issues](https://github.com/vernesong/OpenClash/issues)；内核侧（协议/TUN/DNS 解析/规则引擎）搜 [Mihomo Issues](https://github.com/MetaCubeX/mihomo/issues)。优先参考维护者回复（OpenClash 的 vernesong、Mihomo 的 Contributor/Collaborator）、高赞社区答复，以及其中的诊断命令（可直接复用）。 |
| **本文档是知识库** | 回答需基于本文档与外部权威来源，不得编造；信息来自本文档时注明章节号（如 `14-diagnostics.md` §14.2），来自外部时注明来源链接。 |
...
| meta-rules-dat | `https://github.com/MetaCubeX/meta-rules-dat` | GeoSite/GeoIP 分类数据。**master** 分支：构建脚本 + README（各分类来源、`@cn` 变体、rule-set 示例）；**meta** 分支：mihomo 编译产物（`geo/geosite/`、`geo/geoip/` 下 `.mrs` 规则集）——写 `GEOSITE`/`GEOIP` 规则前先浏览该目录确认分类名，`.mrs` 地址（`.../meta/geo/geosite/<分类>.mrs`）可直接作 rule-providers 下载 URL |
...
> **调取方式**：本地 skill / Copilot 用 `read_file` 读 `openclash-user-guide/NN-xxx.md`（仓库内即 `.github/skills/openclash-user-guide/`）；在线 AI（ChatGPT / Claude 网页版）用 WebFetch 抓 raw URL `https://raw.githubusercontent.com/vernesong/OpenClash/dev/.github/skills/openclash-user-guide/NN-xxx.md`；把该路径里的 `raw.githubusercontent.com/…/dev/` 换成 `github.com/…/blob/dev/` 就是 GitHub 页面链接（供用户点击，或 AI 无法联网时让用户自取内容粘贴）。

| 用户问题类型 | 子文档 | 内容（关键小节） |
|------|--------|------|
| 理解整体原理 / 启动流程 | `01-architecture.md` | 系统整体架构分层、UCI 配置根、开机启动链路、热生效与重启的差异 |
| 启动失败 / 依赖缺失 | `02-dependencies.md` | §2.1–2.6 内核与依赖清单、启动前的依赖检查逻辑、启动失败故障速查表 |
...
| 本文档未覆盖的查询 | `99-out-of-scope.md` | 外部资源优先级与主动查询流程 |

---

## 不同 agent 如何加载子文档

> **总原则：先按「文档路由表」定位，再只加载命中的 1–2 个子文档，不要一次全读所有子文档。** 路径与 raw URL 见上表「调取方式」；`allowed-tools`（frontmatter，实验性）只影响部分 agent 的工具预授权范围，不影响读文件能力。

| 类别 | 代表 agent | 加载方式 |
|------|-----------|----------|
| 本地/CLI（支持技能目录） | Claude Code / Cursor / Windsurf / Roo Code / Continue / Kiro / Trae / OpenHands | 自动发现技能 → 读 `SKILL.md` → 按需读命中的 `NN-*.md`；路径：安装目录 `~/.claude/skills/openclash-user-guide/`，或插件 `~/.claude/plugins/<owner>/<name>/skills/…`；仓库内为 `.github/skills/openclash-user-guide/` |
| 本地/CLI（按规则文件加载） | Codex CLI / OpenCode / Gemini CLI | 读 `AGENTS.md`/`CLAUDE.md` 或 `.codex`/`.opencode` 规则定位；无自动技能发现时**显式让 AI 读 `SKILL.md`**，再按其 Read/Bash 按相对路径读子文档 |
| VS Code Copilot | GitHub Copilot | 自动发现 `.github/skills/<name>/SKILL.md`（本仓库即此路径），用 `read_file` 读子文档 |
| 网页/在线（无本地文件） | ChatGPT / Claude 网页版、任意带联网能力的 agent | 用 WebFetch 抓上表 raw URL；抓不到则请用户**粘贴该子文档内容**（不要让其一次贴入全部子文档），再按「内容（关键小节）」列的 `§` 号定位 |

---

## 子文档总览

> 子文档清单以「文档路由表」为准（上表已含全部 18 个子文档及其关键小节），此处不再重复列表。
